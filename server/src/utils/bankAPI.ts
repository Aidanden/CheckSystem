...
export class BankAPIClient {
  ...
  constructor() {
    this.baseUrl = process.env.BANK_API_URL || 'http://10.250.100.40:8080/FCUBSAccService';
  ...
  private async postAccountInfoSoapRequest(envelope: string): Promise<string> {
    try {
      ...
      try {
        ...
        endpoint = process.env.BANK_IA_API_URL || 'http://10.250.100.40:8080/FCUBSIAService';
      }
